 ...
 * Descricao: Sistema de logistica, envolvendo produtos e encomendas, com diversas funcoes que atuam sobre estas duas entidades.
 */
...
char clientes[MAX_ENC][MAX_NOME];
...
void add_encomenda (int contador_enc)
{
    ...
    scanf(" %s", cliente);
    strcpy(clientes[contador_enc], cliente);
    printf ("Nova encomenda %d %s.\n", contador_enc, cliente);
}

void lista_cliente(int contador_enc)
{
    int ide;
    scanf(" %d", &ide);
    if (ide < contador_enc)
...
void merge(int flag, int esq, int meio, int dir)
{
    ...
    for (k = esq; k <= dir; k++)
    {
        if (flag == 1)
            ...
            cmp = custos[aux[j]] < custos[aux[i]];
        if (cmp || (flag == 1 && produtos[aux[j]].preco == produtos[aux[i]].preco && aux[j] < aux[i]) || (flag == 2 && custos[aux[j]] == custos[aux[i]] && aux[j] < aux[i]))
            ord[k] = aux[j--];
        else
            ord[k] = aux[i++];
    }
...
void enc_ord_custo (int contador_prod, int contador_enc)
{
    int i, j, k, l;
    puts ("Encomendas");
    for (i = 0; i < contador_enc; i++)
    {
        ord[i] = i;
        for (k = 0; k < contador_prod; k++)
            custos[i] += (encomendas[i][k] * produtos[k].preco);
    }
    mergesort (2, 0, contador_enc-1);
    for (j = 0; j < contador_enc; j++)
        printf ("* %d %d\n", ord[j], custos[ord[j]]);
    for (l = 0; l < contador_enc; l++)
        custos[l] = 0;
}
    ...
    while ((comm = getchar()) != 'x') /* Introducao do comando. */
    {
        switch (comm)
        {
            ...
            case 'L':
                ...
                break;
            case 'V':
                lista_cliente(contador_enc);
                break;
            case 'Y':
                enc_ord_custo(contador_prod, contador_enc);
                break;
        }
    }
    return 0;
